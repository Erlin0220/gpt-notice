...
const { RUNTIME_FILES, buildExtension, runtimeSource } = require("../scripts/build-extension");
...
test("production manifest is local and contains one page controller", () => {
  ...
  const scrollStabilizer = fs.readFileSync(path.join(root, "scroll-stabilizer.js"), "utf8");
  assert.match(scrollStabilizer, /ResizeObserver/);
  assert.match(scrollStabilizer, /requestAnimationFrame/);
  assert.doesNotMatch(scrollStabilizer, /MutationObserver|IntersectionObserver|setInterval|setTimeout|HTMLElement\.prototype|Element\.prototype/);
  const longChat = fs.readFileSync(path.join(root, "longchat-perf.js"), "utf8");
  const longChatCss = fs.readFileSync(path.join(root, "longchat-perf.css"), "utf8");
  assert.doesNotMatch(longChat, /MutationObserver|IntersectionObserver|setInterval|setTimeout/);
  assert.match(longChat, /chatgpt-accelerator-hidden/);
  assert.match(longChatCss, /content-visibility:\s*auto/);
  assert.match(longChatCss, /contain-intrinsic-size:\s*auto\s+500px/);
  assert.doesNotMatch(longChatCss, /content-visibility:\s*hidden/);
  ...
  assert.match(content, /recoverProbeTurn\(message, scope, url, p\)/);
  assert.match(content, /observedOutcome\(p, active\)/);
  ...
  // and the fail-open CSS long-chat optimization.
  ...
  assert.ok(scripts.includes("longchat-perf.js"));
  assert.ok(scripts.includes("scroll-stabilizer.js"));
  assert.ok(m.content_scripts.some(group => group.css?.includes("longchat-perf.css") && group.run_at === "document_idle"));
...
test("build runtime allowlist cannot escape the repository through a linked directory", () => {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "gpt-notice-project-"));
  const outside = fs.mkdtempSync(path.join(os.tmpdir(), "gpt-notice-outside-"));
  try {
    fs.writeFileSync(path.join(outside, "secret.txt"), "outside");
    fs.symlinkSync(outside, path.join(project, "linked"), process.platform === "win32" ? "junction" : "dir");
    assert.throws(() => runtimeSource(project, "linked/secret.txt"), /越出仓库/);
  } finally {
    fs.rmSync(project, { recursive: true, force: true });
    fs.rmSync(outside, { recursive: true, force: true });
  }
});
test("CI and releases share the allowlist and required third-party notice", () => {
  assert.ok(RUNTIME_FILES.includes("THIRD_PARTY_NOTICES.md"));
  const notices = fs.readFileSync(path.join(root,"THIRD_PARTY_NOTICES.md"),"utf8");
  assert.match(notices,/kartikkabadi\/chatgpt-yolo/);
  assert.doesNotMatch(notices,/tylevnovik\/chatgpt-web-accelerator/);
  for (const workflow of ["ci.yml", "auto-release.yml"]) {
    ...
    assert.match(source,/timeout-minutes:\s+\d+/);
  }
  ...
  assert.match(updater,/\[switch\]\$WhatIf/);
  assert.ok(updater.indexOf("if ($WhatIf)") < updater.indexOf("Move-Item -LiteralPath $TargetPath"));
  ...
  assert.match(updater,/Resolve-Path -LiteralPath/);
  assert.match(updater,/Move-Item -LiteralPath \$backupPath -Destination \$TargetPath/);
