...
2026-09-16 的一组真实浏览器对比中，在同一账号、同一页面版本下刷新并观察约 10 秒：

| 状态 | Fetch/XHR |
| --- | ---: |
| 使用前：原生“项目”展开 | 约 63 次 |
| 使用 gpt-notice：置顶 / 项目 / 聊天默认折叠 | 约 51 次 |

同一轮测试里，从三个分区全部折叠的状态单独展开“项目”，会立即多出约 10 个项目相关请求；前 5 个项目常见就是约 5 个项目详情请求加 5 个项目最近会话请求。这个数字只用于说明当时真实页面中的量级，不能当成当前或未来 ChatGPT Web 的固定请求预算。
