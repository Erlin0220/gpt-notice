...
6. Observe manual and queued native generation. Use native conversation request completion only as a transport-level candidate event, then probe the exact document for semantic completed / attention / failed state. Hidden, frozen or temporarily unresponsive pages may emit a generic no-content notification and later upgrade the same notification ID when DOM semantics are available. Suppress interim notifications while Queue work remains. Never use network completion to settle or advance Queue; failed/stopped/HITL responses are not successful completion.
