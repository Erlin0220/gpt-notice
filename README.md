...
当前账号配置为 50 次额度，GPT-6 Pro 与 GPT-5.6 Sol Pro 共用这组 Chat 额度。扩展仅在本机观察到实际 `gpt-6-pro` / `gpt-5-6-pro` 模型回复时计数；Thinking、Work 和 Codex 不计入。左侧徽标只显示已用次数、额度和下一次刷新时间，不展示“校正/计算”等来源字样，也不伪造官方剩余次数。
