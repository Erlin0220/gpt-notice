(() => {
  ...
  const CSS = `
    ...
    *{box-sizing:border-box}[hidden],:host([hidden]){display:none!important}
    ...
    .usage{min-width:0;max-width:min(380px,65vw);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--n-muted,#666);font-weight:500}.queue{display:flex;align-items:center;gap:5px}.count{display:inline-grid;min-width:18px;height:18px;place-items:center;padding:0 5px;border-radius:999px;background:var(--n-hover,#f0f0f0);font-size:11px}
  ...
  function create(onAction) {
    ...
    function render(next) {
      ...
      for (const name of ["add", "queue"]) $(`[data-action="${name}"]`).hidden = next.mode !== "conversation" || next.queueEnabled === false;
